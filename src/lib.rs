...
fn unwrapped_option_type(ty: &syn::Type) -> std::option::Option<&syn::Type> {
...
fn analyze_fields(f: &syn::Field) -> Option<FieldBuilderMetadata> {
    ...
    if let std::option::Option::Some(a) =
        attrs.iter().find(|a| a.path.segments[0].ident == "builder")
    {
                    ...
                    std::option::Option::Some(syn::NestedMeta::Meta(syn::Meta::NameValue(
                        ...
                        if path.segments[0].ident == "each" {
                            ...
                            if name == ls_id {
                                ...
                                field_info.set_field_code =
                                    std::option::Option::Some(add_set_function);
                                return std::option::Option::Some(field_info);
                            } else {
    ...
    std::option::Option::Some(field_info)
}
...
pub fn derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let field_metadata: std::vec::Vec<FieldBuilderMetadata> =
        fields.iter().map(|f| analyze_fields(f).unwrap()).collect();

    //////////////////////////////////////////////////////////
    // builder structure fields
    let builder_definition_data: std::vec::Vec<_> = field_metadata
        .iter()
        .map(|f| (f.name.clone(), f.inner_type.clone(), f.can_set_each))
        .collect();

    let builder_definition: std::vec::Vec<_> = builder_definition_data
        .iter()
        .map(|(name, inner_type, can_set_each)| {
            if *can_set_each {
    ...
    let names: std::vec::Vec<_> = field_metadata
        .iter()
        .map(|f| (f.name.clone(), f.can_set_each))
        .collect();
    ...
    //////////////////////////////////////////////////////////
    // Builder Methods
    let set_field_funcs: std::vec::Vec<_> = field_metadata
        .iter()
        .map(|f| f.set_field_code.clone())
        .collect();

    let builder_methods: std::vec::Vec<_> = set_field_funcs
        .iter()
        .map(|set_func| {
            quote::quote! {
                 #set_func
    ...
    let optional: std::vec::Vec<_> = field_metadata
        .iter()
        .map(|f| (f.name.clone(), f.optional, f.can_set_each))
        .collect();
    ...
    let output_fields: std::vec::Vec<_> = optional
        .iter()
        .map(|(name, is_optional, can_set_each)| {
            if !is_optional {
                if *can_set_each {
...
pub fn derive_enum_to_dict(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    // get the list of fields from the structure
    let variants = if let syn::Data::Enum(syn::DataEnum { ref variants, .. }) = parsed_copy.data {
        variants
    } else {
        // this dervive (builder) only supports structures at this time
        unimplemented!();
